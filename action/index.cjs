...
function countsLine(report) {
  ...
  return `${summary.bumps} bump${summary.bumps === 1 ? "" : "s"} reviewed, ${summary.critical} critical, ${summary.high} high, ${summary.medium} medium, ${summary.low} low, ${summary.info} informational.`;
}
...
function worstSeverity(report) {
  for (const severity of [...SEVERITY_ORDER].reverse()) {
    for (const review of report.reviews) {
      if (review.findings.some((finding) => finding.severity === severity)) {
        return severity;
      }
    }
  }
...
function shouldFail(report, threshold) {
  const worst = worstSeverity(report);
  return worst !== void 0 && meetsThreshold(worst, threshold);
}

// src/errors.ts
var ActionDiffError = class extends Error {
  code;
...
var GitError = class extends ActionDiffError {
  constructor(message) {
    super("git", message);
    this.name = "GitError";
  }
};
var GitHubError = class extends ActionDiffError {
  status;
...
var MANIFEST_NAMES = ["action.yml", "action.yaml"];
var MAX_FILES = 40;
var MAX_FILE_BYTES = 512 * 1024;
var RestGitHubClient = class {
  #token;
  #fetch;
  #userAgent;
  ...
  async #json(path) {
    ...
    if (this.#token !== void 0 && this.#token !== "") {
    ...
    if (response.status === 403 || response.status === 429) {
      ...
      const hint = remaining === "0" ? " Rate limit exhausted; supply a token via --token or GITHUB_TOKEN." : "";
  ...
  async resolveCommit(slug, ref) {
    const body = await this.#json(
      `/repos/${slug}/commits/${encodeURIComponent(ref)}`
    );
    return body.sha;
  }
  async listTree(slug, sha) {
    const body = await this.#json(`/repos/${slug}/git/trees/${sha}?recursive=1`);
    return (body.tree ?? []).filter((entry) => entry.type === "blob" && typeof entry.path === "string").map((entry) => ({
      path: entry.path,
      ...entry.size === void 0 ? {} : { size: entry.size }
    }));
  }
  async readFile(slug, sha, path) {
    try {
      const body = await this.#json(
        `/repos/${slug}/contents/${path.split("/").map(encodeURIComponent).join("/")}?ref=${sha}`
      );
      if (body.encoding !== "base64" || body.content === void 0) return void 0;
      if ((body.size ?? 0) > MAX_FILE_BYTES) return void 0;
      return Buffer.from(body.content, "base64").toString("utf8");
    } catch (error) {
      if (error instanceof GitHubError && error.status === 404) return void 0;
      throw error;
    }
  }
  async refsContaining(slug, sha) {
  ...
  async defaultBranch(slug) {
    const body = await this.#json(`/repos/${slug}`);
    return body.default_branch;
  }
  async isAncestor(slug, ancestor, descendant) {
    try {
      const body = await this.#json(
...
function selectReviewableFiles(entries, subpath, entryPoints) {
  const prefix = subpath === void 0 || subpath === "" ? "" : `${subpath.replace(/\/$/u, "")}/`;
  ...
  for (const entry of within) {
    ...
    const isInteresting = relative.endsWith(".sh") || relative.endsWith(".bash") || relative === "Dockerfile" || relative.endsWith("/Dockerfile") || relative === "package.json" || relative === "entrypoint.sh";
  ...
  return [...wanted].filter((path) => present.has(path) && !oversize.has(path)).sort().slice(0, MAX_FILES);
}

// src/classify.ts
function runtimeKind(using) {
  if (using === void 0) return "unknown";
...
var URL_PATTERN = /\bhttps?:\/\/([A-Za-z0-9.-]+)/gu;
var SHELL_FETCH = /\b(curl|wget|nc|ncat|Invoke-WebRequest|Invoke-RestMethod)\b/u;
var SECRET_REFERENCE = /\b(secrets\.[A-Za-z0-9_]+|GITHUB_TOKEN|ACTIONS_ID_TOKEN|NODE_AUTH_TOKEN)\b/u;
var EVAL_PATTERN = /\b(eval|Function\s*\(|child_process|execSync|spawnSync|vm\.runIn)\b/u;
var EXPECTED_HOSTS = /* @__PURE__ */ new Set([
  "api.github.com",
  "github.com",
...
function hostsIn(lines) {
  const hosts = /* @__PURE__ */ new Set();
  for (const line of lines) {
    for (const match of line.matchAll(URL_PATTERN)) {
      const host = match[1];
      if (host !== void 0) hosts.add(host.toLowerCase());
    }
  }
  return hosts;
}
function manifestFindings(before, after) {
  const findings = [];
  ...
  if (before.using !== after.using) {
    ...
    findings.push({
      ...
      message: changedCategory ? `Execution model changed from ${before.using ?? "unset"} to ${after.using ?? "unset"}. A different runtime can reach different resources on the runner.` : `Runtime moved from ${before.using ?? "unset"} to ${after.using ?? "unset"}.`,
      path: "runs.using",
      before: before.using,
      after: after.using
    });
  ...
  for (const [name, spec] of Object.entries(after.inputs)) {
    ...
    if (previous === void 0) {
      findings.push({
        ...
        message: spec.required ? `New required input "${name}". Existing callers that omit it will fail.` : `New optional input "${name}".`,
        path: `inputs.${name}`
      });
      continue;
    }
    ...
    if (previous.default !== spec.default) {
      findings.push({
        ...
        severity: "medium",
        message: `Default for "${name}" changed from ${previous.default ?? "unset"} to ${spec.default ?? "unset"}. Callers relying on the old default change behaviour without changing their workflow.`,
        path: `inputs.${name}`,
...
function classifyRevisions(before, after) {
  const findings = [];
  if (before.manifest !== void 0 && after.manifest !== void 0) {
    findings.push(...manifestFindings(before.manifest, after.manifest));
  } else if (before.manifest !== void 0 && after.manifest === void 0) {
    findings.push({
  ...
  for (const [path, content] of Object.entries(after.files)) {
    ...
    const isNewFile = previous === void 0;
  ...
  const newHosts = [...hostsIn(allAdded)].filter((host) => !beforeHosts.has(host) && !EXPECTED_HOSTS.has(host)).sort();
  if (newHosts.length > 0) {
    findings.push({
      code: "source.new-egress-host",
      severity: "high",
      message: `New outbound host${newHosts.length > 1 ? "s" : ""}: ${newHosts.join(", ")}. This revision can reach somewhere the previous one did not.`,
      after: newHosts
    });
  }
...
function readDependencies(content) {
...
function dependencyFindings(before, after) {
  ...
  const added = Object.keys(current).filter((name) => previous[name] === void 0).sort();
...
async function assertRepository(context) {
  await git(context, ["rev-parse", "--git-dir"]);
}
async function resolveCommit(context, revision) {
  try {
    const stdout = await git(context, ["rev-parse", "--verify", `${revision}^{commit}`]);
    return stdout.trim();
  } catch {
    throw new GitError(
      `cannot resolve "${revision}". In CI this usually means a shallow clone; set fetch-depth: 0 on actions/checkout.`
    );
  }
}
async function listWorkflowFiles(context, revision) {
  ...
  return stdout.split("\n").map((line) => line.trim()).filter((line) => line.endsWith(".yml") || line.endsWith(".yaml"));
}
async function readFileAtRevision(context, revision, path) {
  try {
...
function parseActionRef(uses, versionComment) {
  const value = uses.trim();
  if (value === "" || value.startsWith("./") || value.startsWith("../")) return void 0;
  if (value.startsWith("docker://")) return void 0;
  ...
  if (segments.length < 2 || owner === void 0 || repo === void 0) return void 0;
  ...
  return {
    ...
    ref,
    raw: value,
    ...claimed === void 0 ? {} : { versionComment: claimed }
  };
}
function normalizeVersionComment(comment) {
...
// src/detect.ts
async function collectRefs(context, revision, paths) {
  ...
  for (const path of paths) {
    ...
    for (const ref of extractActionRefs(source)) {
      ...
      if (existing === void 0) {
        collected.set(identity, { ref, workflows: /* @__PURE__ */ new Set([path]) });
        continue;
      }
...
async function detectBumps(context, base, head, restrictTo) {
  const [basePaths, headPaths] = await Promise.all([
    listWorkflowFiles(context, base),
    listWorkflowFiles(context, head)
  ]);
  const wanted = restrictTo === void 0 || restrictTo.length === 0 ? void 0 : new Set(restrictTo);
  const paths = [.../* @__PURE__ */ new Set([...basePaths, ...headPaths])].filter((path) => wanted === void 0 || wanted.has(path)).sort();
...
function parseActionManifest(source) {
  let document;
  try {
    document = (0, import_yaml.parse)(source, { logLevel: "silent" });
  } catch {
    return void 0;
  }
  ...
  const inputs = {};
  for (const [name2, raw] of Object.entries(asRecord(root.inputs))) {
    const spec = asRecord(raw);
    const description2 = asString(spec.description);
  ...
  const outputs = {};
  for (const [name2, raw] of Object.entries(asRecord(root.outputs))) {
    const description2 = asString(asRecord(raw).description);
    outputs[name2] = description2 === void 0 ? {} : { description: description2 };
  }
...
function manifestEntryPoints(source) {
  let document;
  try {
    document = (0, import_yaml.parse)(source, { logLevel: "silent" });
  } catch {
  ...
  return candidates.map((value) => asString(value)).filter((value) => value !== void 0 && !value.startsWith("docker://"));
}

// src/provenance.ts
async function verifyProvenance(client, ref) {
  let sha;
  try {
  ...
  return {
    ...
    reachableFrom,
    onDefaultBranch,
    ...tagPointsElsewhere === void 0 ? {} : { tagPointsElsewhere }
  };
}
async function defaultBranchContains(client, slug, sha) {
  try {
...
async function tagMoved(client, ref, sha) {
  const claimed = ref.versionComment;
  if (claimed === void 0 || !isFullSha(ref.ref)) return void 0;
  try {
    const tagSha = await client.resolveCommit(ref.slug, claimed);
    return tagSha === sha ? void 0 : tagSha;
  } catch {
    return void 0;
  }
}
function provenanceFindings(ref, provenance) {
  const findings = [];
  if (provenance.unverified !== void 0) {
    findings.push({
      code: "provenance.unverified",
      severity: "low",
      message: `Could not confirm that ${ref.slug}@${ref.ref} is published upstream: ` + provenance.unverified
    });
    return findings;
  }
  if (!isFullSha(ref.ref)) {
    findings.push({
      code: "pin.not-a-sha",
      severity: "high",
      message: `${ref.raw} is pinned to a mutable reference. A tag or branch can be repointed at different code without the workflow changing.`,
      after: ref.ref
    });
  }
  if (provenance.reachableFrom.length === 0 && !provenance.onDefaultBranch) {
    findings.push({
      ...
      severity: "critical",
      message: `${ref.slug}@${provenance.sha.slice(0, 12)} is not contained in any tag or branch and is not an ancestor of the default branch. A commit outside published history is what a rewritten tag leaves behind.`,
  ...
  if (provenance.tagPointsElsewhere !== void 0) {
    findings.push({
      ...
      severity: "critical",
      message: `${ref.slug} is pinned to ${provenance.sha.slice(0, 12)} and labelled ${ref.versionComment ?? "a release"}, but that tag now points at ${provenance.tagPointsElsewhere.slice(0, 12)}. Either the pin is stale or the tag was moved.`,
...
// src/review.ts
async function loadRevision(client, ref) {
  let sha;
  try {
    sha = isFullSha(ref.ref) ? ref.ref : await client.resolveCommit(ref.slug, ref.ref);
  } catch {
  ...
  const prefix = ref.subpath === void 0 ? "" : `${ref.subpath.replace(/\/$/u, "")}/`;
  const manifestSource = await client.readFile(ref.slug, sha, `${prefix}action.yml`) ?? await client.readFile(ref.slug, sha, `${prefix}action.yaml`);
  const entryPoints = manifestSource === void 0 ? [] : manifestEntryPoints(manifestSource);
  let files = {};
  try {
    const tree = await client.listTree(ref.slug, sha);
    const wanted = selectReviewableFiles(tree, ref.subpath, entryPoints);
    const contents = await Promise.all(
      wanted.map(async (path) => [path, await client.readFile(ref.slug, sha, path)])
    );
    files = Object.fromEntries(
      contents.filter((entry) => entry[1] !== void 0)
    );
  } catch {
    files = {};
  }
  const manifest = manifestSource === void 0 ? void 0 : parseActionManifest(manifestSource);
  return {
    ref,
    sha,
...
async function reviewBumps(client, options) {
  const context = { root: options.root ?? process.cwd() };
  ...
  for (const bump of bumps) {
    ...
    const findings = [];
    const provenance = options.skipProvenance ?? false ? void 0 : await verifyProvenance(client, bump.after);
...
// src/action.ts
function input(name) {
  return (import_node_process.env[`INPUT_${name.replace(/ /gu, "_").toUpperCase()}`] ?? "").trim();
}
function setOutput(name, value) {
  const file = import_node_process.env.GITHUB_OUTPUT;
...
function appendSummary(markdown) {
  const file = import_node_process.env.GITHUB_STEP_SUMMARY;
  if (file === void 0 || file === "") return;
  (0, import_node_fs.appendFileSync)(file, markdown, "utf8");
}
function resolveBase(explicit) {
  if (explicit !== "") return explicit;
  const baseRef = import_node_process.env.GITHUB_BASE_REF;
  if (baseRef !== void 0 && baseRef !== "") return `origin/${baseRef}`;
  return void 0;
}
function parseFailOn(raw) {
  if (raw === "") return "high";
  const allowed = [...SEVERITY_ORDER, "never"];
...
async function main() {
  ...
  if (base === void 0) {
    throw new Error(
      'No base revision. This action derives one from GITHUB_BASE_REF on pull requests; for other events set the "base" input explicitly.'
    );
  }
  const token = input("token") !== "" ? input("token") : import_node_process.env.GITHUB_TOKEN;
  const root = input("root") !== "" ? input("root") : void 0;
  const paths = input("paths").split("\n").map((line) => line.trim()).filter((line) => line !== "");
  const client = new RestGitHubClient(token === void 0 ? {} : { token });
